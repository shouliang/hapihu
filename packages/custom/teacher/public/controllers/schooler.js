(function () {
  ...
  angular
      .module('mean.teacher')
      .controller('SchoolerController', SchoolerController);

  SchoolerController.$inject = ['$scope', 'Global', '$timeout','MeanUser','Task','Books','$uibModal','Classes'];

  function SchoolerController($scope, Global, $timeout,MeanUser,Task,Books,$uibModal,Classes) {
    var vm=this;
    $scope.test=function () {
    ...
    vm.init=function () {
      ...
      //get new examination
      Task.schoolerTasks.get({contentType:'examination',page:1,pageItem:10},function (res) {
        if(res.result){
          $scope.tasks=res.data;
        }
      },function (res) {
      ...
      $scope.addPaper=function () {
        var modalInstance = $uibModal.open({
          animation: true,
          templateUrl: 'teacher/views/schooler/addPaper.html',
...
angular.module('mean.teacher')
    .controller('SchoolerPaperController',function ($scope,$state,$stateParams,Books,$location,MeanUser,$uibModal,Classes,Task) {
      var vm=this;
      vm.selectedNode={};
      vm.init=function () {
        Books.teacherBook.get({}, function(res) {
    ...
    .controller('PaperSettingController',function ($scope,$stateParams,Task,Classes,Books) {
      ...
      $scope.addChecker=function () {
        ...
        Task.taskChecker.save({taskId:$scope.task._id},{checkerUsername:$scope.newChecker.username,to:classes,content:content},function (res) {
          $scope.toasty.success({
            title:" ｡◕‿◕｡",
            msg:"添加成功"
          });
